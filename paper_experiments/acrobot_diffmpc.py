...
from diffmpc.dynamics.acrobot_dynamics import AcrobotDynamics
...
def main(args):
    ...
    problem_params = load_problem_params("acrobot.yaml")
    ...
    solver_params["linesearch_alphas"] = [1.0 * (0.5**i) for i in range(10)]
    ...
    plt.plot(solutions.states[0, :, 1])
    plt.plot(solutions.states[0, :, 2])
    plt.plot(solutions.states[0, :, 3])
    plt.show()
