...
from diffmpc.dynamics.lipm_dynamics import LipmDynamics  # noqa: E402
...
def generate_problem_data(num_batch, seed):
    ...
    min_omega = 9.0
    max_omega = 11.0
    omega = min_omega + np.random.rand(1) * (max_omega - min_omega)
    min_state = -0.1
    max_state = 0.1
    initial_states = min_state + np.random.rand(num_batch, 4) * (max_state - min_state)
    return jnp.array(omega), jnp.array(initial_states)


def sqp_solve_lipm(num_batch, horizon, use_linesearch, warm_start_backward):
    # Problem parameters
    problem_params = load_problem_params("lipm.yaml")
    omega, initial_states = generate_problem_data(num_batch, seed=0)
    print(f"device: {omega.device}")
    diffmpc_horizon = horizon - 1  # diffmpc horizon is N+1
    problem_params["horizon"] = diffmpc_horizon
    problem_params["omega2"] = omega
    problem_params["reference_state_trajectory"] = jnp.zeros((diffmpc_horizon + 1, 4))
    problem_params["reference_control_trajectory"] = jnp.zeros((diffmpc_horizon + 1, 2))
    # problem_params["reference_control_trajectory"][:, 0] = 1e1
    # problem_params["reference_control_trajectory"][:, 1] = 1e1
    # problem_params["weights_penalization_final_state"] = jnp.array([0.0, 0.0, 1e5, 1e5])
    problem_params["weights_penalization_final_state"] = jnp.zeros(4)
    ...
    params = {**problem_params, "dt": 0.05, "omega2": omega}
...
def test_sqp_solve(num_batch, horizon, use_linesearch, warm_start_backward):
    """Test lipm solve for various horizons and batchsizes"""

    solver, sqp_guess, solution, dynamics = sqp_solve_lipm(
